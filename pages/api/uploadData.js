...
async function storeDataInBlockchain(actualFileName, uniqueFileName) {
    const provider = new ethers.providers.JsonRpcProvider(Constants.ALCHEMY_API_URL);
    ...
    if (isStored == false) {
        const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweGZDNDcxOTJhYjBlMDAxZUM1RUQ0YkE3ZDBiOTBhNjk4ODJiNGFjRjciLCJpc3MiOiJ3ZWIzLXN0b3JhZ2UiLCJpYXQiOjE2ODYzNjM5ODg0ODAsIm5hbWUiOiJJUEZTLUNsb3VkU3RvcmFnZS1EZXNjZW50cmFsaXplZCJ9.9vr_gGq6h1h0OTGnhiBvv-sj79ijqpQEN_O9GyPWIwE";
        ...
        // Cambio de estilo
        const style = "background: linear-gradient(to right, #3cc4ec, #000000); color: #fff; border: 1px solid #333; border-radius: 10px; padding: 10px; font-weight: bold;";
        const message = `IPFS Hash is Stored in the Smart Contract this is the URL: https://${storedHash}.ipfs.w3s.link/uploads/${actualFileName}`;
        ...
        const IPFShash = await StorageContract.getIPFSHash(uniqueFileName);
         ...
         // Cambio de estilo
         const style = "background: linear-gradient(to right, #3cc4ec, #000000); color: #fff; border: 1px solid #333; border-radius: 10px; padding: 10px; font-weight: bold;";
         const message = `IPFS Hash is Stored in the Smart Contract this is the URL: https://${IPFShash}.ipfs.w3s.link/uploads/${actualFileName}`;
